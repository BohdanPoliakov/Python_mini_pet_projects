...
def play(word):
    ...
    input('Давай сыграем в мою игру дружище!')
    input('Играем в угадайку слов)))')
    print(display_hangman(6))
    print(word_completion)
    print()

    while not guessed and tries > 0:
        guess = input('Какую первую букву загадаешь друг? \n').lower()
        if len(guess) == 1 and guess.isalpha():
            if guess in guessed_letters:
                print('Ты уже называл эту букву, повторяться друг нельзя')
            elif guess not in word:
                print('Буквы', guess, 'нет в моем слове лошикс <3 \n')
                tries -= 1  # Ошибка в расчете попыток
                guessed_letters.append(guess)
            else:
                print('Тебе повезло', guess, 'есть в твоем слове \n')
                guessed_letters.append(guess)
                word_as_list = list(word_completion)
                choices = [i for i in range(len(word)) if word[i] == guess]
            ...
            if guess in guessed_words:
                print('Ты уже называла слово \n', guess)
            elif guess != word:
                print('слово', guess, 'не верно \n' )
                tries -= 1
        ...
        print()
    if guessed:
        print('Мои поздравления, ты отгодала слово!)')
    else:
        print('Ты не угадала слово, загаданным словом было', word+ '. Может быть когда-нибудь у тебы выйдет)')

again = 'Да'
while again.lower() == 'да':
    word = get_word()  # Ошибка была здесь
    play(word)
